...
So, I have left the original README below, but `/Phase2/ScaffoldingFilesNotModified/` has an unmodified copy of the original scaffolding files.
Sadly, the provided files do not meet our security scans, so I have had to do a lot of changes, so will try to document said changes below lmao

Pre-commit ignore these files completely:
  - `stubs.c`
  - `bogus_main.c`
  - `db.h`
  - `account.h`
...
It also completely ignores the `/Phase2/ScaffoldingFilesNotModified/` folder

Currently the setup is that there is a symlink in `/src/` that points to `/Phase2/ScaffoldingFilesChanged/src/` (`/src/->../Phase2/ScaffoldingFilesChanged/src/`), so using `make` in `/Phase2/ScaffoldingFilesChanged/src` will use the provided `Makefile`, but using it in `/src/` will use the custom one with a *lot* more warnings.

I also made a compile_commands.json in the `/Phase2/ScaffoldingFilesChanged/` using a similar script to the one in `/src/` that should be run by pre-commit too

Idk, I think I did some other things too tbh
## Contents
