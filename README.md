...
- I have setup a GitHub action that automatically checks to see if there is any code that hasn't been formatted with uncrustify when doing a PR - you can either download the zip file it provides on failure which has a git patch you can apply, or (tbh, probably easier) you can just run uncrustify on your conmputer and commit+push changes.

- If you find that non `*.c` or `*.h` files are getting picked up but the action (it *shouldn't*, but I found it liked to pickup PDF files for some reason) let me (Seb) know, and I'll add it to the ignore file/directory list.

#### Markdown files
- Our report needs to match CommonMark spec, and so we also have a formatter for that (ikr) 
